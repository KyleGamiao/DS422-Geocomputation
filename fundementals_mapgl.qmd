...
```
I feel viridis and cvidis are a lot better in terms of color scheme. They're easier for me to see and interpret. I prefer viridis as cividis is a little too bland in terms of variation in color.
...
It changes a lot more depending on the maximum and minimum values that you want to show
...
Highest tax bracket for Hawaiʻi
```{r}

# Load Hawaii tract-level households earning $200,000+
hi_top_income <- get_acs(
  geography = "tract",
  variables = "B19001_017",  # Households with income $200,000 or more
  state = "HI",
  year = 2023,
  geometry = TRUE
)
...
hi_top_scale <- interpolate_palette(
  data = hi_top_income,
  column = "estimate",
  method = "equal",
  n = 4,
...
# Plot with Mapbox GL
mapboxgl(bounds = hi_top_income) |>
  add_fill_layer(
    id = "hi_top_income_tracts",
    source = hi_top_income,
    fill_color = hi_top_scale$expression,
    fill_opacity = 0.5
  ) |>
  add_legend(
    "Households earning $200K+ in Hawaiʻi",
    values = get_legend_labels(hi_top_scale, digits = 0),
    colors = get_legend_colors(hi_top_scale),
    type = "continuous"
  )
...
# Get total population and population below poverty level
hi_poverty <- get_acs(
  geography = "tract",
  variables = c(
    total = "B17001_001",  # Total population with poverty status determined
    poverty = "B17001_002" # Below poverty level
  ),
  state = "HI",
  year = 2023,
  geometry = TRUE
...
hi_poverty_wide <- hi_poverty %>%
  select(GEOID, variable, estimate, geometry) %>%
  tidyr::pivot_wider(names_from = variable, values_from = estimate) %>%
  mutate(poverty_rate = 100 * poverty / total)

# Create a color scale for poverty rate
hi_pov_scale <- interpolate_palette(
  data = hi_poverty_wide,
  column = "poverty_rate",
  method = "equal",
  n = 4,
  palette = viridis
...
# Plot
mapboxgl(bounds = hi_poverty_wide) |>
  add_fill_layer(
    id = "hi_poverty_rate",
    source = hi_poverty_wide,
    fill_color = hi_pov_scale$expression,
    fill_opacity = 0.5
  ) |>
  add_legend(
  "Poverty rate in Hawaiʻi (%)",
  values = get_legend_labels(hi_pov_scale, digits = 1, suffix = "%"),
  colors = get_legend_colors(hi_pov_scale),
  type = "continuous",
  style = list(color = "black", background = "white", padding = 6)
)
