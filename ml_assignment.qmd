...
options(htmlwidgets.dir = "htmlwidgets_cache")  # makes a stable cache folder
```
...
map_plot <- ggplot(hi_acs) +
  geom_sf(aes(fill = snap_pct), color = "white", size = 0.1) +
  scale_fill_viridis_c(
    option = "viridis", direction = -1, name = "SNAP (%)",
    limits = c(0, 100), breaks = seq(0, 100, 10)
  ) +
  labs(
    title = "SNAP Participation by Census Tract — Hawaiʻi (ACS 5-yr, 2023)",
    subtitle = "Percent of households receiving SNAP benefits",
    caption = "Source: American Community Survey (ACS) 5-year estimates, 2023 — via tidycensus"
  ) +
  coord_sf(datum = NA) +
  ...
  theme(
    ...
    plot.caption = element_text(hjust = 0, size = 4, color = "gray40"),
...
# Hilo on the Big Island and Moloka'i.
2.  Are there geographic patterns (islands, urban vs. rural)?
# Rural areas.
3.  What does this tell you about food security in Hawaiʻi?
# Food security is uneven and there is still a large portion of the population that may or may not be lacking access to food security systems like SNAP benefits.
:::
...
mapview(
  ...
  at = seq(0, ceiling(max(hi_acs$snap_pct, na.rm = TRUE)), by = 5),
...
hist_plot <- ggplot(hi_acs, aes(x = snap_pct)) +
  geom_histogram(
    bins = 20,
    fill = "forestgreen",
    color = "white",
...
# A few outliers with very high participation. I'm surprised that there isn't more participation in urban areas on O'ahu.
:::
...
set.seed(123)  # Instead of a random seed so it can be reproduced
...
# The model captures major trends, generally predicting higher SNAP participation in tracts with higher poverty, lower income, or lower educational attainment. It does miss important factors such as local food programs, household composition, policy differences, cultural influences, and recent economic shocks. This might affect/influence the variation.
...
# Prepares the Random Forest so SHAP can interpret it.
2.  **Calculate SHAP values**: Compute explanations for a subset of observations (computing for all can be slow)
# Measures the contribution of each predictor for a subset of data
3.  **Interpret**: Use the results to explain individual predictions and global patterns
# Shows which features are generally most important/influential and why a particular tract has high or low predicted SNAP participation.
------------------------------------------------------------------------

## 🎯 CHALLENGE 5: Prepare SHAP Calculations
...
## 📊 CHALLENGE 6: Feature Importance Plot
...
feat_plot <- plot_feature_importance(treeshap1, max_vars = 5) +
  ggtitle("Top 5 Features Predicting SNAP Participation (SHAP Values)") +
  theme_minimal(base_size = 8) +
  theme(
...
# Bachelors or higher and poverty rate.
2.  Why do you think these variables matter?
# It's saying that you have a higher chance of being a SNAP participant if you fall in either of the two categories.
3.  Are there any surprising results?
# I'm surprised that unemployment rate is at the bottom.
4.  What does this tell you about food security in Hawaiʻi?
# SNAP use is highest in poorer communities.
:::

------------------------------------------------------------------------

## 📈 CHALLENGE 7: Feature Dependence Plots
...
```{r}
#| label: challenge-7-dependence-1
#| code-fold: false

plot_feature_dependence(treeshap1, "poverty_rate_pct") +
  ggtitle("SHAP Dependence: Poverty Rate") +
...
```{r}
#| label: challenge-7-dependence-2
#| code-fold: false

plot_feature_dependence(treeshap1, "median_income") +
  ggtitle("SHAP Dependence: Median Income") +
  theme_minimal(base_size = 14) +
  theme(plot.title = element_text(hjust = 0.5, face = "bold"))
...
```{r}
#| label: challenge-7-dependence-3
#| code-fold: false

plot_feature_dependence(treeshap1, "bachelors_or_higher_pct") +
  ggtitle("SHAP Dependence: Education (Bachelors or Higher)") +
  theme_minimal(base_size = 14) +
  theme(plot.title = element_text(hjust = 0.5, face = "bold"))
...
```{r}
#| label: challenge-7-dependence-4
#| code-fold: false

plot_feature_dependence(treeshap1, "unemployment_rate_pct") +
  ggtitle("SHAP Dependence: Unemployment Rate") +
  theme_minimal(base_size = 14) +
  theme(plot.title = element_text(hjust = 0.5, face = "bold"))
...
#3.  **Pattern**: Is it linear, or are there non-linear patterns?
     # Non-linear
#4.  **Outliers**: Are there interesting outlier tracts?
     # Yes. At like 36 there's a 0.2
```

```{r}
#| label: challenge-7-dependence-5
#| code-fold: false

plot_feature_dependence(treeshap1, "no_vehicle_pct") +
  ggtitle("SHAP Dependence: No Vehicle Households") +
  theme_minimal(base_size = 14) +
  theme(plot.title = element_text(hjust = 0.5, face = "bold"))
...
## 👁️ CHALLENGE 8: Individual Tract Contribution
...
```{r}
#| label: challenge-8-contribution
#| code-fold: false

plot_contribution(treeshap1, obs = 154) +
  ggtitle("Feature Contributions to SNAP Prediction: Maui Census Tract 304.04") +
  theme_minimal(base_size = 14) +
  theme(plot.title = element_text(hjust = 0.5, face = "bold"))
```
...
Above since its 13% and the baseline was about 11%
5.  **Story**: Write 2-3 sentences telling the "story" of this tract's SNAP participation
In Maui Census Tract 304.04, higher poverty levels, more renters, and limited vehicle access contribute to a higher predicted rate of SNAP participation. While having a bachelors degree or higher contributes to a lower predicted rate of SNAP participation.
:::
...
The Random Forest model explained about 58% of the variation in SNAP participation. It worked well in general, but missed some social and local factors not measured in the Census.
...
Poverty rate, education level, and median income were the biggest predictors. Higher poverty raised SNAP rates, while higher education and income lowered them.
...
SNAP participation was highest in rural and neighbor island areas like Hilo and Molokaʻi. This shows some communities face more economic hardship and food insecurity.
...
The model didn’t include cost of living, food access, or cultural factors. It also can’t prove cause and effect, only show relationships that we feed it.
...
Programs should focus on rural and neighbor island areas with high SNAP use. Increasing access to food, transportation, and job support could reduce food insecurity.
...
SHAP values helped explain how each variable affected the model’s predictions. It showed which factors pushed SNAP rates higher or lower.

7.  **Next Steps**: If you were to continue this analysis, what would you do next?

I would test separate models for each island. Like I'm thinking of them as their own systems since the way of life on each island can/may vary. I'd also like to include real food access data that could make the results more accurate.
:::
...
# Increase right margin for extra space
map_plot <- map_plot + theme(plot.margin = unit(c(10, 40, 10, 10), "pt"))
hist_plot <- hist_plot + theme(plot.margin = unit(c(10, 40, 10, 10), "pt"))
feat_plot <- feat_plot + theme(plot.margin = unit(c(10, 40, 10, 10), "pt"))

# Combine with patchwork
combined <- (map_plot + hist_plot + plot_layout(widths = c(1, 1))) / 
            feat_plot + 
  plot_layout(heights = c(2, 1)) +
  plot_annotation(
    title = "Visual Story of SNAP Participation in Hawaiʻi",
    subtitle = "Combining spatial, distributional, and feature importance",
    theme = theme(
...
combined + theme(plot.background = element_rect(fill = "white"))
```
