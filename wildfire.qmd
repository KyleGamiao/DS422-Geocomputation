...
```{r}
# Aggregate fire counts per day
fires_by_day <- wildfire_shp_data %>%
  st_drop_geometry() %>%        # Drop geometry so we only work with attributes
  group_by(ACQ_DATE) %>%        # Group by acquisition date
  summarise(num_fires = n()) %>%
  arrange(ACQ_DATE)
```

```{r}
# Line plot of fires over time
ggplot(fires_by_day, aes(x = ACQ_DATE, y = num_fires, group = 1)) +
  geom_line(color = "firebrick", linewidth = 1) +
  geom_point(color = "black") +
  labs(
    title = "Daily Wildfire Detections (Landsat - Last 7 Days)",
    x = "Date of Detection",
    y = "Number of Fires"
  ) +
  theme_minimal()
...
```{r}
# Aggregate fire counts by day and confidence level
...
wildfire_area_data <- st_read(
  "data/fire_kml/LANDSAT_USA_contiguous_and_Hawaii_7d_1757466462060.kml",
...
# Read CSV
wildfires_df <- read_csv(here("data", "LANDSAT_USA_contiguous_and_Hawaii_7d.csv"))

# Convert to sf
wildfires_sf <- st_as_sf(
  wildfires_df,
  coords = c("longitude", "latitude"),
  crs = 4326
)

# Assign numeric size for plotting
wildfires_sf <- wildfires_sf %>%
  mutate(CONFIDENCE_NUM = case_when(
    confidence == "low" ~ 1,
    confidence == "medium" ~ 2,
    confidence == "high" ~ 3
  ))

# Get US states shapefile (medium resolution)
usa <- ne_states(country = "united states of america", returnclass = "sf")

# Plot bubble map over USA shape
ggplot() +
  geom_sf(data = usa, fill = "grey95", color = "white") +
  geom_sf(data = wildfires_sf, aes(color = confidence, size = CONFIDENCE_NUM, alpha = CONFIDENCE_NUM)) +
  scale_color_manual(values = c("low" = "#1a9850", "medium" = "#fee08b", "high" = "#d73027")) +
  scale_size_continuous(range = c(1, 6), guide = "none") +
  scale_alpha_continuous(range = c(0.4, 0.9), guide = "none") +
  coord_sf(xlim = c(-125, -66), ylim = c(24, 50)) +  # Focus on continental US
  labs(
    title = "Wildfire Detections Across the USA (Landsat, Last 7 Days)",
    subtitle = "Point size and opacity reflect confidence level",
